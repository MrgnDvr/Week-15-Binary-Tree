...
{
BinaryNode* targetNodePtr = removeValue(subTreePtr->getLeftChildPtr(),
target, success);
...
{
BinaryNode* targetNodePtr = findNode(treePtr->getLeftChildPtr(),
target, success);
if (!success) // no need to search right subTree
{
targetNodePtr = findNode(treePtr->getRightChildPtr(), target,
success);
} // end if
